...
are reported rather than mistaken for a missing cache. Unchanged snapshots skip Nix evaluation, and
new pushes queue behind active builds instead of cancelling uploads.
...
Plasma builds live in [plasma-cache](https://github.com/jmspwr/desktop-cache/tree/main/plasma); the old manual Plasma
workflow has been removed from this repository.

Pull requests run credential-free checks:
...
Source tracking fails explicitly if a selected component changes to a recipe the updater cannot
track, rather than silently retaining an older commit.
